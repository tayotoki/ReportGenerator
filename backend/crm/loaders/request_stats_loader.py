    ...
    def load_data_from_crm(self):
        for table in self.client.get_tables(
            ...
            for data in raw_data:
                if data[self.converter.FIELDS_MAPPER["number"]] in request_numbers:
                    requests_co_update.update(**self.converter(data=data))
                else:
                    requests_to_create.append(self.converter(data=data))
